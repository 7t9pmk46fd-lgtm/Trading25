...
rd-agent's swing positions, so one bad intraday day can't block the
...
Every signal is re-checked here, even signals from day-trading-agent that
...
- `stop_price` is computed by `rd-agent/scripts/generate_signal.py` as
...
`rd-agent/scripts/run_cycle.py` and
`day-trading-agent/scripts/run_sneaky_pivot_cycle.py` — no longer purely
...
- Daily bars used for ATR (both here and in rd-agent/day-trading-agent
