...
- The only way an idea from this agent reaches the execution agent is:
  1. A human reviews the research note (`scripts/review_notes.py`).
  2. If it seems worth pursuing, someone writes an actual, coded strategy
     in `rd-agent/screeners/`.
  3. That strategy gets backtested like any other (see rd-agent).
  4. Only then can it generate real signals.

## Components
- `ingest/news.py` — fetches and extracts readable text from a news article URL.
- `ingest/youtube.py` — fetches a YouTube video's transcript.
- `extract.py` — sends raw text to Claude, gets back a structured summary
  (strategy summary, tickers mentioned, specificity, short excerpt).
- `scripts/ingest_source.py` — end-to-end: URL in, research note stored.
- `scripts/review_notes.py` — list or interactively triage unreviewed notes.
- `scripts/daily_review.py` (added 2026-07-27) — generates a daily PDF
...
python scripts/ingest_source.py https://www.reuters.com/some-article
python scripts/ingest_source.py https://www.youtube.com/watch?v=XXXXXXXXXXX

# Review what's been collected
python scripts/review_notes.py
python scripts/review_notes.py --interactive
```
...
First-ever real run of `ingest/youtube.py` turned up a real bug:
...
transcript successfully). `ingest/news.py`'s real-API path is still
unverified.
